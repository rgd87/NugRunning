...
local function AddActivation( id, opts)
    opts.localname = GetSpellInfo(id)
    if not opts.localname then print("nrun: misssing spell #"..id) return end
...
AddSpell({ 34936 },{ name = "Backlash",duration = 8, shine = true })
...
AddSpell({ 1490 },{ name = "Curse of Elements",duration = 300, color = colors.PURPLE, pvpduration = 120, short = "CoE" })
-- JINX ID 85547
...
AddSpell({ 17 },{ name = "Power Word: Shield", shinerefresh = true, duration = 30, maxtimers = 3, color = colors.LRED, short = "PW:S" })
AddSpell({ 41635 },{ name = "Prayer of Mending", shinerefresh = true, duration = 30, color = colors.RED, textfunc = function(spellName, dstName) return dstName end })
...
-- DEBUFFS
AddSpell({ 589 },{ name = "Shadow Word: Pain",duration = 18, color = colors.PURPLE, refreshed = true, tick = 3, short = "SW:Pain" })
AddSpell({ 34914 },{ name = "Vampiric Touch", recast_mark = "gcd", duration = 15, color = colors.RED, short = "VampTouch", hasted = true })
AddSpell({ 2944 },{ name = "Devouring Plague",duration = 24, color = colors.CURSE, short = "Plague", hasted = true })
...
--~     return ((6 + NugRunning.cpWas *3) * (1+Talent(14165)*0.25))
--~     end })
...
AddSpell({ 94009 },{ name = "Rend", color = colors.RED, duration = 15,
    init = function(self)self.duration = 15 end })   -- now using same model as DKs frost fever & plague
...
AddActivation( 86346, { name = "Reset", shine = true, timeless = true, color = colors.BLACK, duration = 0.1 })
...
AddCooldown( 85288, { name = "Raging Blow",  color = colors.RED })
...
AddActivation( 34428, { name = "Victory Rush", color = colors.PURPLE, duration = 20})
end
